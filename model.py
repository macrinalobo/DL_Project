...
def vae_loss2_cosine(reconstructed_x, x, mean, logvar, closest_neighbors, furthest_neighbors, lambda_kl=1e-2, lambda_nl=1e-2):
    # MSE = F.mse_loss(reconstructed_x, x, reduction='sum')
    # recon_loss = nn.functional.binary_cross_entropy(recon_x, x, reduction='sum')
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    target = torch.ones(reconstructed_x.shape[0],device=device)
    cosine_loss = 1000 * F.cosine_embedding_loss(reconstructed_x, x, target=target, reduction='sum')
    KL_div = -0.5 * torch.sum(1 + logvar - mean.pow(2) - logvar.exp())
    neigh_loss = compute_neighborhood_loss(mean, closest_neighbors, furthest_neighbors)
    return cosine_loss + lambda_kl * KL_div + lambda_nl * neigh_loss
